...
- check http://master-server-node:60010/master-status#requestStats
- create table and put some record
```
$ hbase shell
...
```
- reload the above web page and identify which node increases the write counter. That node has the region.
- now, delete the table and compare how the value changed
    - check the counter
```
